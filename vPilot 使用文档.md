...
您需要在 `Equipment Suffix` 中选择的您飞机的设备代码。请务必选择正确的设备后缀，以便管制员了解您航空器的导航能力。如果您不知道您飞机的设备代码，你可以参考[此处] ***（论坛更新后加入1554Equipment帖子超链接）***。
...
您可以通过右键管制员列表中的管制员然后点击 `Open Private Chat` 来开启私信。这将创建一个新选项卡（或切换到现有选项卡，如果有），然后您可以输入您的消息。**注意：所有管制员指令应在 `com1` 或 `com2` 中发送回复，不应在私信窗口回复，除非您无法在频率中联系到管制员。**
...
> *解决方法：首先请检查您网络是否存在丢包等现象，若您的网络正常，一般是由于国内的网络环境引起，通常使用加速器可以解决。*
