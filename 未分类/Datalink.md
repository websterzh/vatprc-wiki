...
### 如何在模拟飞行中使用CPDLC
***请注意，模拟飞行中的CPDLC功能和以上章节所描述的有较大不同。模拟飞行中的CPDLC某种意义上可以说是真实世界中CPDLC服务的简化版本，但其仍需要遵循真实中CPDLC的通讯规范。由于当前各机模厂商对CPDLC功能实现情况不一，部分机模在支持CPDLC的管制席位之间移交可能需要手动重新登陆。***
#### 如何判断当前管制席位是否支持CPDLC
在模拟飞行中，区域管制员可以根据自身意愿启用CPDLC服务，不受真实中航路和地区的限制。也就是说，在中国大陆空域内进行模拟飞行时，有机会在任意一个情报内接受CPDLC服务。如果区域管制员启用了CPDLC功能，其会将AFTN登录代码和注意事项填写在ATC info中：

```
"Guangzhou Control" CN/EN | CPDLC ZGZU
...
```
该段ATC info表示当前广州区调空域内支持CPDLC服务，登录的AFTN代码为ZGZU。

#### 使用CPDLC进行通讯
当管制员接受（accept）了机组的CPDLC登录请求后，机组将会正式连接到空地CPDLC服务网络上。请注意，在使用CPDLC通讯时，机组仍需按照中国民用航空局规定的CPDLC使用规范和程序进行通讯，参见上一节。有关不同机型使用CPDLC的方法，请跳转至特定机型的章节或参考机模手册。

#### CPDLC服务终止
当航空器离开CPDLC空域进入非CPDLC空域，或是因其他原因CPDLC服务需要终止时，管制员会同时通过数据链和VHF语音对话告知航空器驾驶员。
> *“国航931，CPDLC服务终止，联系阿拉木图133.0，再见。”*

>*"Air China 931, CPDLC service terminated, contact ALmaty Control on 133.0, goodday."*
