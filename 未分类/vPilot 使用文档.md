...
在首次运行 vPilot 时，会弹出一个对话框要求您对vPilot进行配置，单击 `是(Y)` 打开设置窗口并配置 vPilot。
...
选择麦克风设备后，调整麦克风音量滑块，同时以正常声音讲话，使音量大部分处于绿色区域。这将有助于确保您可以被网络上的其他机组和管制员清楚地听到。**注意：若您音量设置的过高，可能会导致其他机组或管制员听力受损。**

如果您勾选了 `Disable realistic ATC audio effects` ，则vPilot输出的声音将不会有 VHF 无线电噪音。通常，为了使音频真实不会选中此框。但是，如果您很难理解 ATC 所说的内容，勾选此项可能会对您有所帮助。

如果您勾选了 `Disable HF background noise` ，则vPilot输出的声音将不会有 HF（高频） 无线电噪音。通常，为了使音频真实不会选中此框。但是，由于HF（高频）无线电的特性，可能会出现背景噪音远大于其他机组或管制员的声音的情况（例如在北大西洋飞行时）。所以当您很难理解或听清ATC所说的内容时，选中此框可能会对您有所帮助。
### Push-To-Talk (PTT)
...
请注意，如果您的PTT使用的是键盘按键，当您以管理员身份运行 FSX/P3D/MSFS时，必须同样以管理员身份运行 vPilot 才可以保证 vPilot 正确识别该按键。
...
此部分允许您在模型匹配中排除模型（单独或作为整个文件夹）。您还可以通过使用 `Move Up` 和 `Move Down` 按钮在此列表中重新排列文件夹来确定文件夹的优先级。vPilot 将首先搜索列表中较高的文件夹用于显示您在 VATSIM 上飞行时遇到的航空器的模型。
...
在这里，您可以限制 vPilot 显示在您模拟器里的航空器数量。如果您在航空器较多的区域发现帧率下降，你可以通过降低这个值来减少您模拟器中显示的航空器数量。
### Updates
...
在这里，您可以选择让 vPilot 在您的航空器离地时自动开启应答机的C 模式。如果您的航空器集成有 SquawkBox 应答机，则应不选中此复选框，因为 vPilot 将能够直接从航空器面板读取应答机模式。
...
![QQ图片20220427233104](https://user-images.githubusercontent.com/104274235/165555393-3a478ff5-c714-4f03-91a4-a96cd618e1c7.png)
...
您可以在`Settings` > `Model Matching` > `Custom Rules` 页面单机 `Add Custom Rule Set(s)` 按钮，加载您的客制化模型匹配规则。您可以手动创建规则文件，也可以从网络上下载。
...
**译者注：由于与CoC中的[B4](/../master/COC_translate_full.md#b4)项产生冲突，请机组依照CoC中的规定，在航空器运动过程中启用应答机的C模式（除非管制员要求将应答机调至Standby）。**
...
这三个选项也适用于 squawk ident。当您第一次联系某个频率时，管制员可能会要求您 应答机识别 。这会使您在管制员的雷达上以不同的方式被显示出来，以便于管制员找到您的位置。如果您被要求 应答机识别，您可以单击航空器面板上的 ident 按钮（如果航空器具有 SquawkBox 转发器集成），或者单击 vPilot 主窗口上的 Ident 按钮。您也可以单击 模拟器 菜单栏 Add-ons 中的 `Squawk Ident` 来实现此功能。当 vPilot 将 ident 信号发送到网络时，vPilot 主窗口上的 `Ident` 按钮将会亮起。

## 与管制员沟通

当您连接到 VATSIM 时，范围内的所有管制员的呼号和频率将显示在 vPilot 主窗口左侧的管制员列表中，并按管制员席位的类型分组。当您将鼠标悬停在条目上时，将会显示该管制员的姓名。

为了与管制员建立通讯，您需要在航空器的无线电面板中调整 `COM1` 或 `COM2` 无线电频率，同时激活无线电发送和接收开关。在 vPilot 主窗口的右上角，当该 COM 无线电激活发送和接收时，您将看到 `TX/RX` 变为白色。
...
您可以将 `COM1` 调至一个管制员的频率，将 `COM2` 调至另一个管制员的频率，并且您可以同时守听多个管制员频率，但同时只能在其中一个管制员频率内发送音频。
...
如果接收的文本消息专门针对您，则会发出提示音并且消息以白色显示。针对其他机组（或特别针对任何人）的文本无线电消息将以灰色显示。
...
如果您在接入 VATSIM 时输入了 SELCAL 代码，那么管制员可以使用该代码向您的航空器发送 SELCAL 提示音。SELCAL主要使用在嘈杂的 HF 频率。机组通常会调低音量，这样他就不会受到 HF 噪音的干扰，当管制员需要通过 HF 与他交流时，管制员会发送 SELCAL 提示音以引起他的注意。VATSIM 不开设 HF 频率，但出于现实目的，会通过VHF模拟HF频率且海洋管制员仍将模拟 SELCAL 过程。
...
要创建和提交飞行计划，请按 vPilot 主窗口上的 `Flight Plan` 。您将看到截图中显示的窗口。此表格将包含您在上一次飞行期间最后在表格内输入的文字。即使未连接到 VATSIM 连飞网络，您也可以填写该表格，但在连接到VATSIM连飞之前，您将无法提交飞行计划。

如果您选中 `Heavy Aircraft` 复选框，vPilot 将在您的航空器类型代码前加上 `H/` ，以向管制员表明您正在驾驶尾流等级为重型的航空器。

您需要在 `Equipment Suffix` 中选择的您航空器的设备代码。请务必选择正确的设备后缀，以便管制员了解您航空器的导航能力。如果您不知道您航空器的设备代码，你可以参考[此处] ***（论坛更新后加入1554Equipment帖子超链接）***。
...
在您使用 VATSIM 的飞行过程中，您可能会想要通过私信联系其他机组或管制员，或者他们可能通过私信与您联系。当您收到新的私信时，vPilot 将会添加一个选项卡，显示发送人的呼号，并浅蓝色突出显示。当您单击该选项卡时，将切换聊天的消息区域并显示私信内容，并且选项卡文本将变为浅灰色。聊天消息的第一行将会显示用户的连线姓名。如果在选项卡关闭后再次收到该用户新消息，呼号将再次以浅蓝色显示。

如果您要回复私信，请在聊天选项卡底部的命令行中输入您的消息，然后按下回车键。您发送的消息将以浅灰色显示，收到的消息将以浅蓝色显示。

您可以通过右键管制员列表中的管制员然后单机 `Open Private Chat` 来开启私信。这将创建一个新选项卡（或切换到现有选项卡，如果有），然后您可以输入您的消息。**注意：所有管制员指令应在 `com1` 或 `com2` 中发送回复，不应在私信窗口回复，除非您无法在频率中联系到管制员。**

您还可以使用 `.chat` 命令发起私信。有关详细信息，请参阅有关[. 命令](#-命令)的部分。
...
| .towerview [IP 地址] [呼号] | 以Observer模式将 vPilot 连接到代理服务器（例如 Euroscope 提供的），以创建机场塔台视角。IP 地址默认为 127.0.0.1 (localhost)，呼号默认为 TOWER。 | .towerview [127.0.0.1] [TOWER] / .towerview |
| .aircraft                   | 开启一个新窗口以显示该区域内每架航空器的详细信息。           |                                             |
| .debug                      | 显示调试窗口。                                               |                                             |
| .ignore 呼号                | 将指定呼号的航空器添加到忽略列表中。否则，vPilot 会渲染列表中的任何航空器的航空器模型。 |                                             |
| .unignore 呼号              | 从忽略列表中删除指定呼号的航空器。                           |                                             |

## 断开与 VATSIM 的连接

当您准备好与 VATSIM 断开连接时，只需按下 vPilot 主窗口上的 `Disconnect` 。此时管制员列表将被清空，并且 vPilot 添加到 模拟器 的所有航空器都将被删除。您也将会断开与语音服务器的连接。
...
vPilot 支持以 Observer 连接，这样您的航空器就不会出现在网络上的其他用户面前。此功能旨在用于共享驾驶舱操作。要使用此功能，第一个飞行员应该正常连接到网络，第二个飞行员应该以 Observer 模式连接。第二个飞行员必须使用与第一个飞行员相同的呼号，并在末尾附加一个字母。

例如，如果第一个飞行员的呼号是CCA081，那么第二个飞行员应该使用 CCA081A。（任何字母都可以，不一定是 A）。通过这样做，第二个飞行员的 vPilot 副本将不会在 模拟器 中显示第一个飞行员的航空器。

## 常见问题

在此罗列一些 vPilot 的常见问题：

### Network Error, Disconnected from network.

![QQ图片20220429145152](https://user-images.githubusercontent.com/104274235/165897533-10f4274e-9d01-4405-87cd-4f533dc14250.png)

此段文字意为：
- 为因为您的网络在一段时间内没有正确响应，或者建立连接失败，导致与 VATSIM 主机 连接失败。
- 断开连接与 VATSIM主机 的连接。
- 断开连接与 VATSIM语音服务器 的连接。

> *解决方法：如偶尔出现，可无需在意；但如果较为频繁，那么请首先请检查您网络是否存在丢包等现象，若您的网络正常，一般是由于国内的网络环境引起，通常可以通过使用加速器解决。*
...
此段文字意为：
- 已断开与语音服务器的连接。原因：服务器连接丢失。
- 将会尝试自动重新连接到语音服务器。
- 连接到语音服务器。

> *解决方法：如偶尔出现，可无需在意，vPilot 会尝试自动重连语言服务器；但如果出现较为频繁，那么请首先请检查您网络是否存在丢包等现象，若您的网络正常，一般是由于国内的网络环境引起，通常可以通过使用加速器解决。*

### Authentication failed

![QQ图片20220428113318](https://user-images.githubusercontent.com/104274235/165897490-88ca1053-9dcd-4440-ba1a-373f4577f7b3.png)

此段文字意为：
- 身份验证失败，驳回连接请求

> *解决方法：可以尝试重启 vPilot ，如果仍然无法连接，一般是由于网络问题导致 VATSIM 服务器无法验证您的账号密码，那么首先请检查您网络是否存在丢包等现象，若您的网络正常，一般是由于国内的网络环境引起，通常可以通过使用加速器解决。*

### Command failed.

![QQ图片20220427233213](https://user-images.githubusercontent.com/104274235/165555664-14b9b65d-6d92-4196-8511-a7fd3902255d.png)
...
![QQ图片20220427233624](https://user-images.githubusercontent.com/104274235/165564820-4cb1f321-0873-4d43-92a1-4100e441a2e7.png)

此段文字意为：
- 无效 `.` 命令

> *解决方法：`.` 命令 错误，请参阅 [此处](#-命令) 查找错误。*
